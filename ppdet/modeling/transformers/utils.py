...
def radarcamera_deformable_attention_core_func(radar_value, camera_value, radar_value_spatial_shapes,
                                   radar_value_level_start_index, camera_value_spatial_shapes, camera_value_level_start_index, radar_sampling_locations,
                                               camera_sampling_locations_list, camera_mask_list, all_attention_weights):
    """
    Args:
        value (Tensor): [bs, value_length, n_head, c]
        value_spatial_shapes (Tensor|List): [n_levels, 2]
    ...
    """
    # radar sampling
    radar_bs, _, n_head, c = radar_value.shape
    _, Len_q, _, n_levels, n_points, _ = radar_sampling_locations.shape

    radar_split_shape = [h * w for h, w in radar_value_spatial_shapes]
    radar_value_list = radar_value.split(radar_split_shape, axis=1)
    radar_sampling_grids = 2 * radar_sampling_locations - 1
    radar_sampling_value_list = []
    for level, (h, w) in enumerate(radar_value_spatial_shapes):
        # N_, H_*W_, M_, D_ -> N_, H_*W_, M_*D_ -> N_, M_*D_, H_*W_ -> N_*M_, D_, H_, W_
        radar_value_l_ = radar_value_list[level].flatten(2).transpose(
            [0, 2, 1]).reshape([radar_bs * n_head, c, h, w])
        # N_, Lq_, M_, P_, 2 -> N_, M_, Lq_, P_, 2 -> N_*M_, Lq_, P_, 2
        radar_sampling_grid_l_ = radar_sampling_grids[:, :, :, level].transpose(
            [0, 2, 1, 3, 4]).flatten(0, 1)
        # N_*M_, D_, Lq_, P_
        radar_sampling_value_l_ = F.grid_sample(
            radar_value_l_,
            radar_sampling_grid_l_,
            mode='bilinear',
            padding_mode='zeros',
            align_corners=False)
        radar_sampling_value_list.append(radar_sampling_value_l_)
    # (N_, Lq_, M_, L_, P_) -> (N_, M_, Lq_, L_, P_) -> (N_*M_, 1, Lq_, L_*P_)
    attention_weights = all_attention_weights[:,:,:,:3,:].transpose([0, 2, 1, 3, 4]).reshape(
        [radar_bs * n_head, 1, Len_q, n_levels * n_points])
    radar_output = (paddle.stack(
        radar_sampling_value_list, axis=-2).flatten(-2) *
              attention_weights).sum(-1).reshape([radar_bs, n_head * c, Len_q])

    #camera sampling
    camera_total, camera_len, n_head, c = camera_value.shape
    _, Len_q, _, n_levels, n_points, _ = camera_sampling_locations_list[0].shape

    camera_split_shape = [h * w for h, w in camera_value_spatial_shapes]
    camera_output = []
    num_cameras = len(camera_sampling_locations_list)
    camera_value = camera_value.reshape(
        [radar_bs, num_cameras, camera_len, n_head, c]).transpose([1, 0, 2, 3, 4])
    for i in range(num_cameras):
        camera_value_list = camera_value[i].split(camera_split_shape, axis=1)
        camera_sampling_grids = 2 * camera_sampling_locations_list[i] - 1
        camera_sampling_value_list = []
        for level, (h, w) in enumerate(camera_value_spatial_shapes):
            # N_, H_*W_, M_, D_ -> N_, H_*W_, M_*D_ -> N_, M_*D_, H_*W_ -> N_*M_, D_, H_, W_
            camera_value_l_ = camera_value_list[level].flatten(2).transpose(
                [0, 2, 1]).reshape([radar_bs * n_head, c, h, w])
            # N_, Lq_, M_, P_, 2 -> N_, M_, Lq_, P_, 2 -> N_*M_, Lq_, P_, 2
            camera_sampling_grid_l_ = camera_sampling_grids[:, :, :, level].transpose(
                [0, 2, 1, 3, 4]).flatten(0, 1)

            # mask_l_ = camera_mask_list[i][:, :, :, level].transpose([0, 2, 1, 3, 4]).flatten(0, 1)
            mask_l_ = camera_mask_list[i][:, :, :, level].transpose([0, 2, 1, 3, 4])
            N, M, Lq, P, C = mask_l_.shape
            mask_l_ = mask_l_.reshape([N * M, Lq, P, C])

            invalid_value = paddle.full_like(camera_sampling_grid_l_, 10.0)
            camera_sampling_grid_l_ = paddle.where(mask_l_.astype('bool'), camera_sampling_grid_l_, invalid_value)

            # N_*M_, D_, Lq_, P_
            camera_sampling_value_l_ = F.grid_sample(
                camera_value_l_,
                camera_sampling_grid_l_,
                mode='bilinear',
                padding_mode='zeros',
                align_corners=False)
            camera_sampling_value_list.append(camera_sampling_value_l_)
        # (N_, Lq_, M_, L_, P_) -> (N_, M_, Lq_, L_, P_) -> (N_*M_, 1, Lq_, L_*P_)
        attention_weights = all_attention_weights[:, :, :, 3:, :].transpose([0, 2, 1, 3, 4]).reshape(
            [radar_bs * n_head, 1, Len_q, n_levels * n_points])
        camera_output.append((paddle.stack(
            camera_sampling_value_list, axis=-2).flatten(-2) *
                        attention_weights).sum(-1).reshape([radar_bs, n_head * c, Len_q]))
    output = radar_output + sum(camera_output) / num_cameras
...
def get_contrastive_denoising_training_group_rotated(targets,
    ...
    for i in range(bs):
        num_gt = num_gts[i]
        if num_gt > 0:
            input_query_class[i, :num_gt] = targets["gt_class"][i].squeeze(-1).astype('int32')
            input_query_bbox[i, :num_gt] = targets["gt_rbox"][i][:,:4]
            input_query_angle[i, :num_gt] = targets["gt_rbox"][i][:,4].unsqueeze(-1)
            pad_gt_mask[i, :num_gt] = 1
