...
class HybridEncoder(nn.Layer):
    ...
    def _reset_parameters(self):
        if self.eval_size:
            for idx in self.use_encoder_idx:
                ...
                # Build the cached eval-time position embedding on CPU first to
                # avoid large cuBLAS allocations during model initialization.
                pos_embed = self.build_2d_sincos_position_embedding_numpy(
    ...
    def build_2d_sincos_position_embedding(w,
                                           ...
                                           embed_dim=256,
                                           temperature=10000.):
        return HybridEncoder.build_2d_sincos_position_embedding_numpy(
            w, h, embed_dim, temperature)

    @staticmethod
    def build_2d_sincos_position_embedding_numpy(w,
                                                 h,
                                                 embed_dim=256,
                                                 temperature=10000.):
        ...
        grid_w, grid_h = np.meshgrid(
            np.arange(int(w), dtype=np.float32),
            np.arange(int(h), dtype=np.float32),
            indexing='xy')
        pos_dim = embed_dim // 4
        omega = np.arange(pos_dim, dtype=np.float32) / pos_dim
        omega = 1. / (temperature**omega)

        out_w = np.matmul(grid_w.reshape(-1, 1), omega.reshape(1, -1))
        out_h = np.matmul(grid_h.reshape(-1, 1), omega.reshape(1, -1))
        pos_embed = np.concatenate(
            [
                np.sin(out_w), np.cos(out_w), np.sin(out_h), np.cos(out_h)
            ],
            axis=1)[None, :, :]
        return paddle.to_tensor(pos_embed, dtype='float32')
...
@register
@serializable
class RadarHybridEncoderP2(nn.Layer):
    __shared__ = ['depth_mult', 'act', 'trt', 'eval_size']
    __inject__ = ['encoder_layer']

    def __init__(self,
                 in_channels=[64, 256, 512, 1024, 2048],
                 feat_strides=[2, 4, 8, 16, 32],
                 hidden_dim=256,
                 use_encoder_idx=[3],
                 num_encoder_layers=1,
                 encoder_layer='TransformerLayer',
                 ...
                 trt=False,
                 eval_size=None):
        super(RadarHybridEncoderP2, self).__init__()
        self.in_channels = in_channels
        self.feat_strides = feat_strides
        self.hidden_dim = hidden_dim
        self.use_encoder_idx = use_encoder_idx
        ...
        self.detail_downsample = BaseConv(
            hidden_dim, hidden_dim, 3, stride=2, act=act)
        self.lateral_convs = nn.LayerList([
    ...
    def _reset_parameters(self):
        if self.eval_size:
            for idx in self.use_encoder_idx:
                stride = self.feat_strides[idx]
                pos_embed = HybridEncoder.build_2d_sincos_position_embedding_numpy(
                    self.eval_size[1] // stride, self.eval_size[0] // stride,
                    self.hidden_dim, self.pe_temperature)
                setattr(self, f'pos_embed{idx}', pos_embed)

    def _encode_level(self, feat, level_idx, encoder_idx):
        b, _, h, w = feat.shape
        ...
        if self.training or self.eval_size is None:
            ...
            pos_embed = getattr(self, f'pos_embed{level_idx}', None)
            if pos_embed is None or pos_embed.shape[1] != h * w:
                pos_embed = HybridEncoder.build_2d_sincos_position_embedding(
                    w, h, self.hidden_dim, self.pe_temperature)
        memory = self.encoder[encoder_idx](src_flatten, pos_embed=pos_embed)
        return memory.transpose([0, 2, 1]).reshape([-1, self.hidden_dim, h, w])

    def forward(self, feats, for_mot=False):
        assert len(feats) == len(self.in_channels)
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]
        if self.num_encoder_layers > 0:
            for encoder_idx, feat_idx in enumerate(self.use_encoder_idx):
                proj_feats[feat_idx] = self._encode_level(
                    proj_feats[feat_idx], feat_idx, encoder_idx)

        feat_s2, feat_s4, feat_s8, feat_s16, feat_s32 = proj_feats
        feat_s4 = feat_s4 + self.detail_downsample(feat_s2)

        top_s16 = self.lateral_convs[0](feat_s32)
        top_s16 = F.interpolate(top_s16, size=feat_s16.shape[-2:], mode='nearest')
        feat_s16 = self.fpn_blocks[0](paddle.concat([top_s16, feat_s16], axis=1))

        top_s8 = self.lateral_convs[1](feat_s16)
        top_s8 = F.interpolate(top_s8, size=feat_s8.shape[-2:], mode='nearest')
        feat_s8 = self.fpn_blocks[1](paddle.concat([top_s8, feat_s8], axis=1))

        top_s4 = self.lateral_convs[2](feat_s8)
        top_s4 = F.interpolate(top_s4, size=feat_s4.shape[-2:], mode='nearest')
        feat_s4 = self.fpn_blocks[2](paddle.concat([top_s4, feat_s4], axis=1))

        pan_s8 = self.pan_blocks[0](
            paddle.concat([self.downsample_convs[0](feat_s4), feat_s8], axis=1))
        pan_s16 = self.pan_blocks[1](
            paddle.concat([self.downsample_convs[1](pan_s8), feat_s16], axis=1))
        return [feat_s4, pan_s8, pan_s16]
    ...
    def out_shape(self):
        return [
            ...
            for stride in self._out_strides
        ]
