...
class ResNet(nn.Layer):
    ...
    def forward(self, inputs, flag = 0):
        if flag == 0:
            ...
            x = inputs['radar_image']
        elif flag == 'camera':
            x = inputs['camera_image']
            if len(x.shape) == 5:
                x = paddle.reshape(x, [-1, x.shape[2], x.shape[3], x.shape[4]])
...
class ResNetRadarP2(ResNet):
    ...
    def __init__(self,
                 ...
                 std_senet=False,
                 freeze_stem_only=False):
        super(ResNetRadarP2, self).__init__(
            depth=depth,
            ch_in=ch_in,
            variant=variant,
        ...
        self._stem_channels = 64

    @property
    def out_shape(self):
        shapes = [ShapeSpec(channels=self._stem_channels, stride=2)]
        shapes.extend([
            ShapeSpec(
                channels=self._out_channels[i], stride=self._out_strides[i])
            for i in self.return_idx
    ...
    def forward(self, inputs, flag='radar'):
        x = inputs['radar_image'] if flag == 'radar' else inputs[flag]
        conv1 = self.conv1(x)
        x = F.max_pool2d(conv1, kernel_size=3, stride=2, padding=1)
        outs = [conv1]
        for idx, stage in enumerate(self.res_layers):
