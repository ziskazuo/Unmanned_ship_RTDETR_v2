...
def gbb_form(boxes):
    ...
    # Guard against degenerate widths/heights so downstream covariance terms stay valid.
    wh = paddle.clip(paddle.abs(wh), min=1e-6)
...
def probiou_loss(pred, target, eps=1e-3, mode='l1', cost=False):
    ...
    t2 = paddle.clip(t2, min=eps)
    t3_denom = paddle.clip(4 * paddle.sqrt(F.relu(t3_)) + eps, min=eps)
    t3_ratio = paddle.clip(t2 / t3_denom, min=eps)
    t3 = 0.5 * paddle.log(t3_ratio)

    B_d = (t1 / t2) + t3
    # B_d = t1 + t2 + t3
    B_d = paddle.nan_to_num(B_d, nan=100.0, posinf=100.0, neginf=eps)
    B_d = paddle.clip(B_d, min=eps, max=100.0)
    l1 = paddle.sqrt(1.0 - paddle.exp(-B_d) + eps)
    l_i = paddle.pow(l1, 2.0)
    l2 = -paddle.log(1.0 - l_i + eps)
    l1 = paddle.nan_to_num(l1, nan=1.0, posinf=1.0, neginf=0.0)
    l2 = paddle.nan_to_num(l2, nan=100.0, posinf=100.0, neginf=0.0)
