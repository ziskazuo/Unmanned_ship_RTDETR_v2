...
__all__ = [
    ...
    'RBoxMetric', 'RBoxCocoStyleMetric', 'SNIPERCOCOMetric'
]
...
class RBoxCocoStyleMetric(Metric):
    def __init__(self, anno_file, **kwargs):
        self.anno_file = anno_file
        self.clsid2catid, self.catid2name = get_categories('RBOX', anno_file)
        self.catid2clsid = {v: k for k, v in self.clsid2catid.items()}
        ...
        self.evaluate_difficult = kwargs.get('evaluate_difficult', False)
        self.imid2path = kwargs.get('imid2path', None)
        # By default, also report class-agnostic ship-only metrics during eval.
        # This reuses the same predictions/GT and does not require extra forward.
        self.enable_ship_only_eval = kwargs.get('enable_ship_only_eval', True)
        iou_thresholds = kwargs.get('iou_thresholds', None)
        if iou_thresholds is None:
            iou_thresholds = [round(0.50 + 0.05 * i, 2) for i in range(9)]
        self.iou_thresholds = [float(v) for v in iou_thresholds]
        self.reset()
    ...
    def reset(self):
        ...
        self.gt_by_image = {}
        self.eval_results = {}
        self.ap_by_threshold = {}
        self.ship_ap_by_threshold = {}
    ...
    def update(self, inputs, outputs):
        ...
        if 'scale_factor' in inputs:
            scale_factor = inputs['scale_factor'].numpy() if isinstance(
                inputs['scale_factor'], paddle.Tensor) else inputs['scale_factor']
        else:
            scale_factor = np.ones((len(gt_boxes), 2)).astype('float32')
        ...
        for i in range(len(gt_boxes)):
            ...
            gt_label = gt_labels[i].numpy() if isinstance(
                gt_labels[i], paddle.Tensor) else gt_labels[i]
            gt_box, gt_label, _ = prune_zero_padding(gt_box, gt_label)
            self.gt_by_image[int(im_id[i])] = (gt_box, gt_label)
    ...
    def _evaluate_threshold(self, threshold, class_agnostic=False):
        class_num = 1 if class_agnostic else len(self.catid2name)
        catid2name = {0: 'ship'} if class_agnostic else self.catid2name
        metric = DetectionMAP(
            class_num=class_num,
        ...
        for result in self.results:
            pred_by_image[int(result['image_id'])].append(result)

        image_ids = set(pred_by_image.keys()) | set(self.gt_by_image.keys())
        for image_id in sorted(image_ids):
            gt_box, gt_label = self.gt_by_image.get(
                image_id,
                (np.zeros((0, 8), dtype='float32'), np.zeros((0,), dtype='int32')))
            if class_agnostic and gt_label.shape[0] > 0:
                gt_label = np.zeros_like(gt_label, dtype='int32')
            dets = pred_by_image.get(image_id, [])
            bbox = [res['bbox'] for res in dets]
            score = [res['score'] for res in dets]
            if class_agnostic:
                label = [0 for _ in dets]
            else:
                label = [
                    self.catid2clsid[int(res['category_id'])] for res in dets
                ]
            metric.update(bbox, score, label, gt_box, gt_label)
        metric.accumulate()
    ...
    def accumulate(self):
        ...
        for threshold in self.iou_thresholds:
            self.ap_by_threshold[f'{threshold:.2f}'] = self._evaluate_threshold(
                threshold)
            if self.enable_ship_only_eval:
                self.ship_ap_by_threshold[f'{threshold:.2f}'] = \
                    self._evaluate_threshold(threshold, class_agnostic=True)

        ap50 = self.ap_by_threshold.get('0.50', 0.0)
        ap75 = self.ap_by_threshold.get('0.75', 0.0)
        mean_ap = float(sum(self.ap_by_threshold.values()) / max(
            len(self.ap_by_threshold), 1))
        # Keep `bbox` for compatibility with the existing best-model selection
        # path, but expose the stricter thresholds with explicit names so logs
        # and dashboards do not mislabel AP50 as mAP.
        self.eval_results = {
            'bbox': [ap50],
            'bbox_AP50': [ap50],
            'bbox_AP75': [ap75],
            'bbox_mAP50_90': [mean_ap],
        }

        if self.enable_ship_only_eval:
            ship_ap50 = self.ship_ap_by_threshold.get('0.50', 0.0)
            ship_ap75 = self.ship_ap_by_threshold.get('0.75', 0.0)
            ship_mean_ap = float(sum(self.ship_ap_by_threshold.values()) /
                                 max(len(self.ship_ap_by_threshold), 1))
            self.eval_results.update({
                'bbox_ship_AP50': [ship_ap50],
                'bbox_ship_AP75': [ship_ap75],
                'bbox_ship_mAP50_90': [ship_mean_ap],
            })
    ...
    def log(self):
        if not self.eval_results:
            return
        logger.info(
            "RBox multi-threshold integral AP50 = {:.2f}%, AP75 = {:.2f}%, "
            "mAP50:90 = {:.2f}%".format(
                100. * self.eval_results['bbox'][0],
                100. * self.eval_results['bbox_AP75'][0],
                100. * self.eval_results['bbox_mAP50_90'][0]))
        if self.enable_ship_only_eval and 'bbox_ship_AP50' in self.eval_results:
            logger.info(
                "RBox ship-only integral AP50 = {:.2f}%, AP75 = {:.2f}%, "
                "mAP50:90 = {:.2f}%".format(
                    100. * self.eval_results['bbox_ship_AP50'][0],
                    100. * self.eval_results['bbox_ship_AP75'][0],
                    100. * self.eval_results['bbox_ship_mAP50_90'][0]))
