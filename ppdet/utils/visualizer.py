...
def draw_bbox(image, im_id, catid2name, bboxes, threshold):
    ...
    for dt in np.array(bboxes):
        ...
        if catid not in catid2color:
            ...
            if catid in predefined_colors:
                catid2color[catid] = np.array(predefined_colors[catid])
            else:
                idx = len(catid2color) % len(color_list)
                catid2color[catid] = color_list[idx, 0:3]
        ...
        if hasattr(draw, 'textbbox'):
            bbox_text = draw.textbbox((xmin, ymin), text)
    ...
    with open(save_path, 'w') as f:
        if "bbox_res" in results:
            for dt in results["bbox_res"]:
                ...
                cat_name = catid2name.get(catid, str(catid))
                bbox_pred = '{} {} '.format(cat_name,
                                            score) + ' '.join(
                                                [str(e) for e in bbox])
                f.write(bbox_pred + '\n')
...
def draw_segm(image,
    ...
    for dt in np.array(segms):
        ...
        if not draw_box:
            ...
            center_x = xs.mean()
            center_y = ys.mean()
            label_text = "{}".format(catid2name.get(catid, str(catid)))
            ...
            bbox_text = '%s %.2f' % (catid2name.get(catid, str(catid)), score)
