...
from ppdet.modeling.architectures.multi_detr_missing_v5 import Discriminator
...
class Trainer_gan(object):
    def __init__(self, cfg, mode='train'):
        ...
        if 'model' not in self.cfg:
            ...
            #discriminator
            self.model_dis_vis = Discriminator()
            self.model_dis_ir = Discriminator()
        ...
        if self.mode == 'train':
            ...
            self.optimizer_D_vis = paddle.optimizer.Adam(learning_rate=0.0002, beta1=0.5, beta2=0.999, parameters=self.model_dis_vis.parameters())
            self.optimizer_D_ir = paddle.optimizer.Adam(learning_rate=0.0002, beta1=0.5, beta2=0.999,
                                                         parameters=self.model_dis_ir.parameters())
    ...
    def load_weights(self, weights, ARSL_eval=False):
        if self.is_loaded_weights:
            return
        self.start_epoch = 0
        load_pretrain_weight(self.model, weights, ARSL_eval, mode = 'multi')
    ...
    def train(self, validate=False):
        ...
        model = self.model
        model_dis_vis = self.model_dis_vis
        model_dis_ir = self.model_dis_ir
            ...
            for step_id, data in enumerate(self.loader):
                ...
                if self.use_amp:
                    ...
                    if isinstance(
                        ...
                        in_vis_image = data['vis_image']
                        in_ir_image = data['ir_image']


                        # model forward
                        outputs = model(data) #this indiccate generator
                        g_fake_vis_img = outputs['generate_vis_pic']
                        g_fake_ir_img = outputs['generate_ir_pic']

                            #caculate d_vis loss
                        vis_d_fake_out = model_dis_vis(g_fake_vis_img.detach(), in_ir_image)
                        vis_d_real_out = model_dis_vis(in_vis_image, in_ir_image)
                        ls_D_vis_1 = nn.functional.binary_cross_entropy(vis_d_fake_out,paddle.zeros(vis_d_fake_out.shape))
                        ls_D_vis_2 = nn.functional.binary_cross_entropy(vis_d_real_out,paddle.ones(vis_d_real_out.shape))
                        ls_D_vis = (ls_D_vis_1 + ls_D_vis_2)
                        # self.optimizer_D_vis.clear_grad()
                        ls_D_vis.backward()
                        self.optimizer_D_vis.step()
                        self.optimizer_D_vis.clear_grad()
                            #caculate d_ir loss
                        ir_d_fake_out = model_dis_ir(g_fake_ir_img.detach(), in_vis_image)
                        ir_d_real_out = model_dis_ir(in_ir_image, in_vis_image)
                        ls_D_ir_1 = nn.functional.binary_cross_entropy(ir_d_fake_out,paddle.zeros(ir_d_fake_out.shape))
                        ls_D_ir_2 = nn.functional.binary_cross_entropy(ir_d_real_out,paddle.ones(ir_d_real_out.shape))
                        ls_D_ir = (ls_D_ir_1 + ls_D_ir_2)
                        # self.optimizer_D_ir.clear_grad()
                        ls_D_ir.backward()
                        self.optimizer_D_ir.step()
                        self.optimizer_D_ir.clear_grad()

                        # train generator(with detector)
                        vis_d_out = model_dis_vis(g_fake_vis_img, in_ir_image)
                        ir_d_out = model_dis_ir(g_fake_ir_img, in_vis_image)
                        g_vis_ls = nn.functional.binary_cross_entropy(vis_d_out,paddle.ones(vis_d_out.shape))
                        g_ir_ls = nn.functional.binary_cross_entropy(ir_d_out,paddle.ones(ir_d_out.shape))

                        loss = outputs['loss'] + g_vis_ls + g_ir_ls
                        # model backward
                        loss.backward()

                        #rebuilt output
                        outputs.pop('generate_vis_pic')
                        outputs.pop('generate_ir_pic')
                        outputs['gan_vis_loss'] = g_vis_ls
                        outputs['gan_ir_loss'] = g_ir_ls
                        outputs['discriminator_vis_loss'] = ls_D_vis
                        outputs['discriminator_ir_loss'] = ls_D_ir
                    self.optimizer.step()
