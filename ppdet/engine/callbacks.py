...
class Checkpointer(Callback):
    ...
    @staticmethod
    def _select_metric_key(map_res):
        eval_func = "ap"
        if 'pose3d' in map_res:
            return 'pose3d', eval_func, 'pose3d'
        if 'bbox' in map_res:
            tie_key = 'bbox_mAP50_90' if 'bbox_mAP50_90' in map_res else 'bbox'
            return 'bbox', eval_func, tie_key
        if 'keypoint' in map_res:
            return 'keypoint', eval_func, 'keypoint'
        return 'mask', eval_func, 'mask'

    @staticmethod
    def _primary_tie_margin(cfg, key):
        # For RBOX_COCO_STYLE, `bbox` is AP50. Use a practical margin so
        # near-equal AP50 checkpoints can be ranked by the stricter mAP50:90.
        if key == 'bbox':
            return float(getattr(cfg, 'best_model_ap50_tie_margin', 1e-3))
        return 1e-12

    @staticmethod
    def _is_better(primary_score, tie_score, best_primary, best_tie,
                   primary_margin, tie_eps=1e-12):
        primary_gap = primary_score - best_primary
        if primary_gap > primary_margin:
            return True
        if abs(primary_gap) <= primary_margin and tie_score > best_tie + tie_eps:
            return True
        return False

    @staticmethod
    def format_eval_metric_name(key):
        metric_alias = {
            'bbox': 'bbox-AP50',
            'bbox_AP50': 'bbox-AP50',
            'bbox_AP75': 'bbox-AP75',
            'bbox_mAP50_90': 'bbox-mAP50:90',
            'bbox_ship_AP50': 'ship-only-AP50',
            'bbox_ship_AP75': 'ship-only-AP75',
            'bbox_ship_mAP50_90': 'ship-only-mAP50:90',
            'keypoint': 'keypoint-mAP',
            'mask': 'mask-mAP',
            'pose3d': 'pose3d',
        }
        return metric_alias.get(key, "{}-mAP".format(key))

    @staticmethod
    def _flatten_metric_results(map_res):
        flat_res = {}
        for k, v in map_res.items():
            if isinstance(v, (list, tuple)) and len(v) > 0:
                flat_res[k] = float(v[0])
            elif isinstance(v, (int, float)):
                flat_res[k] = float(v)
        return flat_res

    def _append_eval_history(self, status, metrics_dict):
        if not self.model.cfg.get('record_eval_history', True):
            return
        if not metrics_dict:
            return
        epoch_id = status.get('epoch_id', -1)
        epoch = int(epoch_id) + 1 if epoch_id is not None else None
        sample_num = int(status.get('sample_num', 0))
        cost_time = float(status.get('cost_time', 0.0) or 0.0)
        fps = float(sample_num / cost_time) if cost_time > 0 else 0.0
        ...
        record = {
            ...
            'best_bbox_AP50': float(self.best_ap),
            'best_tie_metric': float(self.best_tiebreak_ap),
        }
        if 'training_staus' in status and status['training_staus'] is not None:
            record['train_status'] = status['training_staus'].get()

        history_file = self.model.cfg.get('eval_history_file',
                                          'eval_history.jsonl')
        history_path = os.path.join(self.save_dir, history_file)
            ...
            if mode == 'train':
                ...
                if 'save_best_model' in status and status['save_best_model']:
                    ...
                    for metric in self.model._metrics:
                        ...
                        merged_eval_results.update(
                            Checkpointer._flatten_metric_results(map_res))
                        key, eval_func, tie_key = self._select_metric_key(map_res)
                        if key == 'pose3d':
                            eval_func = "mpjpe"
                        if key not in map_res:
                            ...
                            continue
                        primary_score = map_res[key][0]
                        tie_score = map_res.get(tie_key, [primary_score])[0]
                        primary_margin = Checkpointer._primary_tie_margin(
                            self.model.cfg, key)
                        better = Checkpointer._is_better(
                            primary_score, tie_score, self.best_ap,
                            self.best_tiebreak_ap, primary_margin)
                        if better:
                            self.best_ap = primary_score
                            self.best_tiebreak_ap = tie_score
                            save_name = 'best_model'
                            weight = self.weight.state_dict()
                        logger.info("Best test {} {} is {:0.3f} (tie-break {}={:0.3f}).".format(
                            key, eval_func, abs(self.best_ap), tie_key,
                            abs(self.best_tiebreak_ap)))
                    self._append_eval_history(status, merged_eval_results)
            if weight:
            ...
            if mode == 'eval':
                for metric in self.model._metrics:
                    for key, map_value in metric.get_results().items():
                        metric_name = Checkpointer.format_eval_metric_name(key)
                        self.vdl_writer.add_scalar("eval/{}".format(metric_name),
                                                   map_value[0],
                                                   self.vdl_mAP_step)
                self.vdl_mAP_step += 1
        ...
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            ...
            if mode == 'eval':
                ...
                for metric in self.model._metrics:
                    for key, map_value in metric.get_results().items():
                        metric_name = Checkpointer.format_eval_metric_name(key)
                        merged_dict["eval/{}".format(metric_name)] = map_value[0]
                ...
                if 'save_best_model' in status and status['save_best_model']:
                    for metric in self.model._metrics:
                        map_res = metric.get_results()
                        key, _eval_func, tie_key = Checkpointer._select_metric_key(map_res)
                        if key not in map_res:
                            logger.warning("Evaluation results empty, this may be due to " \
                                        "training iterations being too few or not " \
                                        "loading the correct weights.")
