...
from ppdet.metrics import Metric, COCOMetric, VOCMetric, WiderFaceMetric, \
    RBoxCocoStyleMetric, get_infer_results, KeyPointTopDownCOCOEval, \
    KeyPointTopDownMPIIEval, Pose3DEval
...
class Trainer(object):
    ...
    def _init_metrics(self, validate=False):
        ...
        if self.cfg.metric == 'COCO' or self.cfg.metric == "SNIPERCOCO":
        ...
        elif self.cfg.metric == 'RBOX_COCO_STYLE':
            bias = self.cfg['bias'] if 'bias' in self.cfg else 0
            output_eval = self.cfg['output_eval'] \
                if 'output_eval' in self.cfg else None
            save_prediction_only = self.cfg.get('save_prediction_only', False)
            imid2path = self.cfg.get('imid2path', None)
            iou_thresholds = self.cfg.get('iou_thresholds', None)
            enable_ship_only_eval = self.cfg.get('enable_ship_only_eval', True)
            ...
            self._metrics = [
                RBoxCocoStyleMetric(
                    ...
                    enable_ship_only_eval=enable_ship_only_eval)
            ]
    ...
    def _assert_finite_loss(self, loss, outputs, epoch_id, step_id):
        if not self.cfg.get('stop_on_nonfinite_loss', True):
            return
        if bool(np.isfinite(loss.numpy()).all()):
            return
        ...
        for k, v in outputs.items():
            if isinstance(v, paddle.Tensor):
                arr = v.numpy()
                if arr.size == 1:
                    scalar_outputs[k] = float(arr.reshape([-1])[0])
        logger.error(
            "Detected non-finite loss at epoch={}, step={}. "
            "Training is stopped to avoid wasting compute. scalar_outputs={}".
            format(epoch_id, step_id, scalar_outputs))
        raise FloatingPointError(
            "Non-finite loss detected at epoch={}, step={}.".format(
                epoch_id, step_id))
                    ...
                    if isinstance(
                        ...
                        with model.no_sync():
                            with paddle.amp.auto_cast(
                                ...
                                outputs = model(data)
                                loss = outputs['loss']
                            self._assert_finite_loss(loss, outputs, epoch_id,
                                                     step_id)
                            # model backward
    ...
    def radar_predict(self,
        ...
        self.dataset.set_images(radar_images,camera_images)
        loader = create('TestReader')(self.dataset, 0)
        ...
        if visualize:
            for outs in results:
                ...
                start = 0
                for i, im_id in enumerate(outs['im_id']):
                    radar_image_path = imid2path[int(im_id)][0]
                    #ir_image_path = imid2path[int(im_id)][1]
                    radar_image = Image.open(radar_image_path).convert('RGB')
                    #ir_image = Image.open(ir_image_path).convert('RGB')
                    radar_image = ImageOps.exif_transpose(radar_image)
                    #ir_image = ImageOps.exif_transpose(ir_image)
                    self.status['original_radar_image'] = np.array(radar_image.copy())
                    ...
                    pose3d_res = batch_res['pose3d'][start:end] \
                            if 'pose3d' in batch_res else None
                    radar_image = visualize_results(
                        radar_image, bbox_res, mask_res, segm_res, keypoint_res,
                        pose3d_res, int(im_id), catid2name, draw_threshold)
                    ...
                    self.status['result_radar_image'] = np.array(radar_image.copy())
                    #self.status['result_ir_image'] = np.array(ir_image.copy())
                    if self._compose_callback:
                        self._compose_callback.on_step_end(self.status)
                    # save image with detection
                    save_radar_name = self._get_save_vis_image_name(output_dir,
                                                          radar_image_path)
                    # save_ir_name = self._get_save_ir_image_name(output_dir,
                    #                                             ir_image_path)
                    logger.info("Detection bbox results save in {}".format(
                        save_radar_name+' '))
                    radar_image.save(save_radar_name, quality=95)
